...
ユーザーが明示的に方向転換しない限り、次に Agent Core、Workflow V1、Reddit saved sync、X live auth verification を作らないでください。Phase 19 link-first baseline が現在の主要 product path です。Stable `link.queue.upsert`、stable `link.media.sync`、public `mediagent link sync <url>`、queue claim/retry scheduling、canonical/media identity dedupe、Reddit external-provider delegation、Redgifs downloads、simple multi-candidate partial-success handling を含みます。Resolver behavior は default で anonymous かつ bounded に保ち、今後の platform work は account/bookmark collectors より explicit-link provider adapters を先に拡張してください。
