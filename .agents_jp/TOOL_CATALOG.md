...
- `jmcomic.library.reconcile`: `plan` は full-library chapter audit のみ。`apply` は `confirm:true` を必須とし、healthy local source pages から affected CBZ だけを rebuild して replaced archive を quarantine する。
...
定期 service では `--json` の代わりに `--summary-json` を使い、journal の詳細出力を抑えます。
...
- `library.trash.reconcile`: Pre-v10 cleanup により移動された downloaded rows を plan または atomic import し、明示的 removed library state にします。Original relative path、recorded size、legacy trash candidate の SHA-256 を検証します。Files は移動せず、duplicate old trash copies を保持し、managed/reconciliation trash を無視し、unmatched row または active identity conflict で apply 全体を block します。Dry-run 対応で、apply 前の review が必須です。
- `library.trash.status`: Current service account が `.trash/mediagent` を安全に書けるか、UID/GID/mode と no-automatic-purge retention policy を含めて report します。Tool `success` は query 成功のみを意味し、caller は `operational` を確認します。
- `library.trash.prepare`: Existing `.trash` の owner/permissions を変えず `.trash/mediagent` だけを create/validate します。Symlink と library 外 path を拒否し、read-only dry-run に対応します。
...
- `pixiv.comics.package`: Complete downloaded Pixiv manga ごとに deterministic Kavita-oriented CBZ を plan/create します。Archive は `comic/<series-directory>/` に置き、zero-padded page names と root-level `ComicInfo.xml` を含み、`comic-kavita-v2` として記録します。One-shot は unique series identity、`Number=1`、`Count=1`、`Format=One-Shot` を使い、real Pixiv series は同じ directory と available series/chapter metadata を共有します。Write は `.partial` と atomic replacement を使います。`migrate_legacy:true` は V1 date-layout archive を rebuild し、old CBZ を `library.entry.remove` で retire します。Old media/source row は explicit removed entry に link したまま保持し、rerun は無視します。Pixiv への接続や source pages の削除は行いません。
