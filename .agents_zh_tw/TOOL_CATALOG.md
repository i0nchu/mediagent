...
- `pixiv.comics.package`：為每個完整下載的 Pixiv 漫畫規劃或建立 deterministic CBZ。頁序依 Pixiv metadata，archive 包含 zero-padded 頁面名稱與 `ComicInfo.xml`，透過 `.partial` 加 atomic replacement 寫入，並以 `comic-cbz-v1` 記錄到 `media_files`。健康且已追蹤的 CBZ 會重用；工具不連線 Pixiv，也不刪除原始頁面。
