...
- `library_content.py`: global SHA-256 content identity, scanner-visible projections, dedup scan/apply, verified legacy-trash reconciliation, and remove/restore/rename lifecycle
...
`content_blobs` owns checksum identity; provider `media_items` and `media_files` keep every source and provenance record. One ordinary-media `library_entry` is scanner-visible for a checksum, so cross-provider duplicates do not appear repeatedly in external scanners. Comic source pages and CBZ archives use context-specific presentation keys because one byte-identical page may legitimately belong to multiple chapters; they keep those paths and use hard links when possible. Removing a library entry records a planned/completed operation and moves its presentation under the symlink-safe `.trash/mediagent/<removal-id>/` namespace without deleting source metadata. Status/prepare operations manage only that namespace, never the legacy trash tree. Repair and verification skip removed entries until an explicit restore. Raw `download.http` remains an unmanaged transport primitive; managed sync tools perform adoption immediately after their DB file upsert.

The library lifecycle CLI is the integration boundary for external catalogs, viewers, and cleanup selectors. Such a service may pass an explicit managed path plus an opaque `external_ref` to `mediagent library remove|restore|rename`, but its API polling, account credentials, selection rules, pagination, and scheduler remain outside this repository. Mediagent never imports those external APIs or decides what their users selected. Reader-compatible files and metadata are output contracts only, not runtime service dependencies.
